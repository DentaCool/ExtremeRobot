...
class ExtremeBot(commands.Bot):
    def __init__(self, bot_token: str, command_prefix, **options):
        super().__init__(command_prefix, **options)
        self.__bot_token = bot_token

    def __str__(self):
        return f'Дискорд бот\nПрефикс:{self.command_prefix}\nТокен:{self.__bot_token}'
    ...
    def start(self):
        ...
        async def on_ready():
            ...
            await self.change_presence(status=discord.Status.idle,
                                       activity=discord.Game(name=f'{self.command_prefix}codewars'))
        ...
        async def check(ctx, username):
            ...
            if not cw.activation_check(username, code):
                ...
                try:
                    ...
                    server = self.get_guild(464822298537623562)
                    roles = {
                        'server': self.get_guild(464822298537623562),
                    ...
                    roles = {
                        'server': self.get_guild(743465592601837679),
                        't0': server.get_role(743466011763933244),
                        't1': server.get_role(743466116382457997),
                        't2': server.get_role(743466149332647976)
                    }
                    '''
                    # cw.get_rank возвращает число ранка. В кодварсе оно отрицательно ибо в будущем будут добавлены
                    # dan ранки
                    ...
                    logging.error(e)
                    await ctx.send(embed=discord.Embed(title=':x: Ой, при проверке профиля произошла ошибка!',
                                                       color=0xf00))
...
if __name__ == '__main__':
    ExtremeBot(bot_token=TOKEN, command_prefix='ex/').start()
